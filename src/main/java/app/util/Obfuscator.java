 ...
 * completely pointless! In case you're looking for a Java code obfuscation tool
 * for this task, please consider ProGuard, available and usable for free at
 * <a href=
 * "http://proguard.sourceforge.net">http://proguard.sourceforge.net</a>.
 * <p>
...
//
   ...
   * string. Obfuscation is performed by encoding the given string into UTF8 and
   * then XOR-ing a sequence of pseudo random numbers to it in order to prevent
   * attacks based on character probability. The result is encoded into an array
   * of longs which is embedded in some Java code which would produce the
   * original string again. The sequence of pseudo random numbers is seeded with
   * a 48 bit random number in order to provide a non-deterministic result for
   * the generated code. Hence, two subsequent calls with the same string will
   * produce equal results by a chance of 1/(2<sup>48</sup>-1) (0 isn't used as
   * a seed) only!
   * <p>
   * As an example, calling this method with {@code "Hello world!"} as its
   * parameter may produce the result {@code "new ObfuscatedString(new long[] {
   *     0x3676CB307FBD35FEL, 0xECFB991E2033C169L, 0xD8C3D3E365645589L
   * }).toString()"}. If this code is compiled and executed later, it will
   * produce the string {@code "Hello world!"} again.
   *
  ...
  public static String obfuscate(final String s) {
    ...
    if (-1 != s.indexOf(0)) {
      throw new IllegalArgumentException(new Obfuscator(
          new long[] { 0x241005931110FC70L, 0xDCD925A88EAD9F37L,
              0x19ADA1C861E2A85DL, 0x9A5948E700FCAD8AL, 0x2E11C83A72441DE2L })
    ...
    final StringBuffer code =
        new StringBuffer(new Obfuscator(new long[] { 0xA28E32BB0D3E394EL,
            0xF842D1C94E549EECL, 0x7D07DFF01F907E4L, 0x4E0BDE791ECD467CL,
            0xDFF389B58DA3E44FL, 0x2477FAED0CE62C79L }).toString()); // => "new
    ...
    code.append(new Obfuscator(new long[] { 0x4200B7AD6FFFF546L,
        0x9B822E95FE73769DL, 0x23C2800C6CACFCE3L, 0x21C30B492D9AEF99L })
            .toString()); // => "}).toString() /* => \"";
    ...
    code.append(s
        .replaceAll("\\\\",
            new Obfuscator(
                new long[] { 0x6D2C680D49523A01L, 0xB932F1DBD19E82CEL })
                    .toString() /* => "\\\\\\\\" */)
        .replaceAll("\"",
            new Obfuscator(
                new long[] { 0x85E9D53EF7A9324BL, 0xB05BD65C9F19DE07L })
                    .toString() /* => "\\\\\"" */));

    code.append(
        new Obfuscator(
        new long[] { 0xC54FFF0621E7D107L, 0x194EAD468C6FCF93L }).toString()); // =>
                                                                              // "\"
                                                                              // */"
   ...
   * index {@code off}. This is the inverse of
   ...
   * index {@code off}. This is the inverse of {@link #toLong(byte[], int)}. If
   * less than eight bytes are remaining in the array, only these low order
   ...
   */
  public Obfuscator(final long[] obfuscated) {
