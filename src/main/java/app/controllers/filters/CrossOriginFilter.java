 ...
 * <dd>a comma separated list of origins that are allowed to access the
 * resources. Default value is <b>*</b>, meaning all origins.
 * <p>
 ...
 * <dd>a comma separated list of origins that are allowed to time the resource.
 * Default value is the empty string, meaning no origins.
 * <p>
 * The check whether the timing header is set, will be performed only if the
 * user gets general access to the resource using the <b>allowedOrigins</b>.
 *
 * <dt>allowedMethods</dt>
 * <dd>a comma separated list of HTTP methods that are allowed to be used when
 * accessing the resources. Default value is <b>GET,POST,HEAD</b></dd>
 * 
 * 
 * <dt>allowedHeaders</dt>
 * <dd>a comma separated list of HTTP headers that are allowed to be specified
 * when accessing the resources. Default value is
 * <b>X-Requested-With,Content-Type,Accept,Origin</b>. If the value is a single
 * "*", this means that any headers will be accepted.</dd>
 * 
 * <dt>preflightMaxAge</dt>
 * <dd>the number of seconds that preflight requests can be cached by the
 * client. Default value is <b>1800</b> seconds, or 30 minutes</dd>
 * 
 * <dt>allowCredentials</dt>
 * <dd>a boolean indicating if the resource allows requests with credentials.
 * Default value is <b>true</b></dd>
 * 
 * <dt>exposedHeaders</dt>
 * <dd>a comma separated list of HTTP headers that are allowed to be exposed on
 * the client. Default value is the <b>empty list</b></dd>
 * 
 * <dt>chainPreflight</dt>
 * <dd>if true preflight requests are chained to their target resource for
 * normal handling (as an OPTION request). Otherwise the filter will response to
 * the preflight. Default is <b>true</b>.</dd>
 * 
 * </dl>
...
public class CrossOriginFilter implements Filter {
  ...
  public void init(FilterConfig config) throws ServletException {
    ...
    anyTimingOriginAllowed =
        generateAllowedOrigins(allowedTimingOrigins, allowedTimingOriginsConfig, DEFAULT_ALLOWED_TIMING_ORIGINS);
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Cross-origin filter configuration: " + ALLOWED_ORIGINS_PARAM + " = " + allowedOriginsConfig + ", "
          + ALLOWED_TIMING_ORIGINS_PARAM + " = " + allowedTimingOriginsConfig + ", " + ALLOWED_METHODS_PARAM + " = "
          + allowedMethodsConfig + ", " + ALLOWED_HEADERS_PARAM + " = " + allowedHeadersConfig + ", "
          + PREFLIGHT_MAX_AGE_PARAM + " = " + preflightMaxAgeConfig + ", " + ALLOW_CREDENTIALS_PARAM + " = "
          + allowedCredentialsConfig + "," + EXPOSED_HEADERS_PARAM + " = " + exposedHeadersConfig + ","
